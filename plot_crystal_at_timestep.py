...
def plot_crystals_at_timestep(
    ...
    ax=None,
    box_size_um=50.0,
    particle_radius_um=1.0,  # not used yet, but accepted for API compatibility
):
    ...
    # Consider a particle as part of a crystal if it has neighbors
    valid_particles = np.sum(valid_distances, axis=1) > 1
    ...
    neighbor_counts = np.array(
        [
            ...
            and -1 not in vor.regions[vor.point_region[i]]
            else 0
    ...
    if ax is None:
        ...
        # make sure pyplot's "current axes" is this one (for voronoi_plot_2d)
        plt.sca(ax)
    ...
    for i, crystal_type in enumerate(crystalline_types):
        ax.scatter(
            positions[i, 0],
            positions[i, 1],
            c=color_map[crystal_type],
            label=crystal_type,
            marker="o",
        )

    # Plot Voronoi on this axes
    voronoi_plot_2d(
        vor,
        show_vertices=False,
        line_colors="black",
    ...
    ax.set_title(f"Crystals at Timestep {timestep*13.513:.2f}s")
    ax.grid(True)
